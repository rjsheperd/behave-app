...
(defn ->class [db [class-name methods]]
  ...
  (let [source-edn (read-string (slurp (fs/expand-home f)))
        ...
        tx         (mapv (fn [[ns-key ns-id]]
                           (merge {:cpp.namespace/name  (->str ns-key)
                                   :cpp.namespace/class (mapv (partial ->class (ds/unwrap-db conn)) (get source-edn ns-key))}
