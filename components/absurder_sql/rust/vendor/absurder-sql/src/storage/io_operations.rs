            ...
            if block_id == 0 {
                let data = vfs_sync::with_global_storage(|gs| {
                    ...
                    #[cfg(target_arch = "wasm32")]
                    {
                        ...
                        crate::utils::debug_log_1(&format!(
...
fn write_block_impl_inner(
    ...
    #[cfg(target_arch = "wasm32")]
    {
        // BEHAVE FIX: SQLite is the sole authority on page contents — its
        // writes through the VFS must ALWAYS be applied. The previous
        // "richer data" heuristic here compared non-zero byte counts and
        // silently DROPPED writes with more zeros than the existing block
        // whenever the block lacked committed metadata (version 0). Restored
        // blocks always have version 0 (persisted metadata rows carry only
        // checksums), so after a page reload every page rewrite that shrank
        // content — freed pages, freelist trunk/leaf updates — was discarded,
        // and the next sync persisted a torn image (integrity_check:
        // "freelist leaf count too big", btreeInitPage SQLITE_CORRUPT).
        ...
        if should_write {
            ...
            // BEHAVE FIX (incremental sync): record the write so sync only
            // persists blocks changed since the last successful persist.
            vfs_sync::mark_block_dirty(&storage.db_name, block_id);
        }
