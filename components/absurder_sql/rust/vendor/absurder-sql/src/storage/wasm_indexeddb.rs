...
async fn restore_from_indexeddb_internal(db_name: &str, force: bool) -> Result<(), DatabaseError> {
    ...
    #[cfg(target_arch = "wasm32")]
    crate::utils::debug_log_1(&format!("[RESTORE] Acquiring IndexedDB mutex...").into());
    ...
    if let Some(_marker) = existing_marker {
        ...
        #[cfg(target_arch = "wasm32")]
        crate::utils::debug_log_1(&format!("[RESTORE] Opening IndexedDB to restore blocks").into());
        ...
        if let Ok(Ok(db)) = rx.await {
            ...
            #[cfg(target_arch = "wasm32")]
            crate::utils::debug_log_1(&format!("[RESTORE] Block restoration complete").into());
            return Ok(());
        }
...
async fn persist_to_indexeddb_event_based_internal(
    ...
    impl Drop for SlotGuard {
        fn drop(&mut self) {
            ...
            crate::utils::debug_log_1(&"[GUARD] Released IndexedDB slot via guard".into());
        }
    }
...
pub async fn delete_blocks_from_indexeddb(
    ...
    impl Drop for SlotGuard {
        fn drop(&mut self) {
            super::indexeddb_queue::release_indexeddb_slot();
            crate::utils::debug_log_1(&"[GUARD] Released IndexedDB slot via guard (delete)".into());
        }
    }
    let _slot_guard = SlotGuard;
    ...
    for block_id in block_ids {
        ...
        #[cfg(target_arch = "wasm32")]
        crate::utils::debug_log_1(&format!("[DELETE] Deleting block key: {}", key).into());
...
pub async fn delete_all_database_blocks_from_indexeddb(db_name: &str) -> Result<(), DatabaseError> {
    ...
    // Acquire queue slot to prevent browser-level IndexedDB contention
    super::indexeddb_queue::acquire_indexeddb_slot().await;
    crate::utils::debug_log_1(&"[DELETE_ALL] Acquired IndexedDB slot".into());
    ...
    impl Drop for SlotGuard {
        fn drop(&mut self) {
            super::indexeddb_queue::release_indexeddb_slot();
            crate::utils::debug_log_1(&"[DELETE_ALL] Released IndexedDB slot via guard".into());
        }
    }
    let _slot_guard = SlotGuard;
    ...
    crate::utils::debug_log_1(&format!("[DELETE_ALL] Deleted {} blocks", blocks_count).into());
