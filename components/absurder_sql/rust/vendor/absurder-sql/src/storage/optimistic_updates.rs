...
impl OptimisticUpdatesManager {
    ...
    pub fn track_write(&mut self, sql: String) -> String {
        ...
        #[cfg(target_arch = "wasm32")]
        crate::utils::debug_log_1(&format!("Tracked optimistic write: {}", id).into());
    ...
    pub fn confirm_write(&mut self, id: &str) {
        if let Some(write) = self.pending_writes.get_mut(id) {
            ...
            #[cfg(target_arch = "wasm32")]
            crate::utils::debug_log_1(&format!("Confirmed optimistic write: {}", id).into());
        }
    }
    ...
    pub fn fail_write(&mut self, id: &str) {
        if let Some(write) = self.pending_writes.get_mut(id) {
            ...
            #[cfg(target_arch = "wasm32")]
            crate::utils::debug_log_1(&format!("Failed optimistic write: {}", id).into());
        }
    }
    ...
    pub fn clear_all(&mut self) {
        #[cfg(target_arch = "wasm32")]
        {
            ...
            crate::utils::debug_log_1(&format!("Cleared {} optimistic writes", count).into());
        }
