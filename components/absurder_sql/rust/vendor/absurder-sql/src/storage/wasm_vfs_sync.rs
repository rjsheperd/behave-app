...
pub fn vfs_sync_database(db_name: &str) -> Result<(), DatabaseError> {
    ...
    // crate::utils::debug_log_1(&"vfs_sync_database() CALLED - THIS SHOULD BE RARE!".into());
    ...
    let (blocks_to_persist, metadata_to_persist) = vfs_sync::with_global_storage(|storage| {
        ...
        //crate::utils::debug_log_1(&format!("VFS sync: GLOBAL_STORAGE keys: {:?}", storage_map.keys().collect::<Vec<_>>()).into());
        //crate::utils::debug_log_1(&format!("VFS sync: Looking for key: {}", db_name_clone).into());

        let blocks = if let Some(db_storage) = storage_map.borrow().get(&db_name_clone) {
            //crate::utils::debug_log_1(&format!("VFS sync: Found {} blocks for {}", db_storage.len(), db_name_clone).into());
            db_storage
                .iter()
...
pub fn vfs_sync_database_blocking(db_name: &str) -> Result<(), DatabaseError> {
    ...
    if blocks_to_persist.is_empty() {
        crate::utils::debug_log_1(&format!("VFS sync: No blocks to persist for {}", db_name).into());
        return Ok(());
    }
...
pub fn sync_blocks_only(storage: &BlockStorage) -> Result<(), DatabaseError> {
    ...
    // crate::utils::debug_log_1(&format!("DEBUG: sync_blocks_only called for {}", _db_name).into());
    ...
    // crate::utils::debug_log_1(&format!("DEBUG: sync_blocks_only completed for {} (commit marker unchanged)", db_name).into());
