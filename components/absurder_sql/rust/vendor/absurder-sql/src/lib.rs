...
impl Database {
    ...
    pub async fn new(config: DatabaseConfig) -> Result<Self, DatabaseError> {
        ...
        let (connection_state, db) = {
            ...
            let state = crate::connection_pool::get_or_create_connection(&pool_key, || {
                ...
                #[cfg(target_arch = "wasm32")]
                crate::utils::debug_log_1(&format!("[OPEN] About to call sqlite3_open_v2...").into());
    ...
    pub async fn sync_internal(&mut self) -> Result<(), DatabaseError> {
        ...
        #[cfg(target_arch = "wasm32")]
        {
            ...
            // BEHAVE FIX (incremental sync): persist only blocks written since
            // the last successful persist instead of every block of the db.
            // The dirty set is taken up front and restored on persist failure
            // so no write is ever lost.
            let dirty_blocks = vfs_sync::take_dirty_blocks(storage_name);
            let (blocks_to_persist, metadata_to_persist) =
                vfs_sync::with_global_storage(|storage| {
                    ...
                    let blocks = if let Some(db_storage) = storage_map.get(storage_name) {
                        crate::utils::debug_log_1(
                            &format!(
                                "[SYNC] {} dirty of {} blocks in GLOBAL_STORAGE",
                                dirty_blocks.len(),
                                db_storage.len()
                            )
                            .into(),
                        ...
                        dirty_blocks
                            .iter()
                            .filter_map(|id| db_storage.get(id).map(|data| (*id, data.clone())))
                            .collect::<Vec<_>>()
                    } else {
                        crate::utils::debug_log_1(
                            &format!(
                    ...
                    let metadata = vfs_sync::with_global_metadata(|meta| {
                        ...
                        if let Some(db_meta) = meta_map.get(storage_name) {
                            dirty_blocks
                                .iter()
                                .filter_map(|id| db_meta.get(id).map(|m| (*id, m.checksum)))
                                .collect::<Vec<_>>()
                        } else {
                            crate::utils::debug_log_1(&format!("[SYNC] No metadata found").into());
                            Vec::new()
                        }
                    });
            ...
            if !blocks_to_persist.is_empty() {
                ...
                let persist_result = crate::storage::wasm_indexeddb::persist_to_indexeddb_event_based(
                    storage_name,
                    blocks_to_persist,
                    metadata_to_persist,
                    next_commit,
                ...
                if let Err(e) = persist_result {
                    // Persist failed — re-mark the blocks dirty so the next
                    // sync retries them (nothing is silently dropped).
                    vfs_sync::restore_dirty_blocks(storage_name, dirty_blocks);
                    return Err(e);
                }
                crate::utils::debug_log_1(
                ...
                crate::utils::debug_log_1(
                    &format!("[SYNC] No dirty blocks to persist — skipping IndexedDB write").into(),
                );
            }
...
impl Drop for Database {
    fn drop(&mut self) {
        crate::utils::debug_log_1(&format!("DROP: Releasing connection for {}", self.name).into());
        ...
        crate::utils::debug_log_1(&format!("DROP: Connection released for {}", self.name).into());
