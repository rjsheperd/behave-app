...
/// Debug-only console log sink. In release builds this is a no-op, so the
/// hot-path diagnostics sprinkled through the VFS/storage layers (one or
/// more per 4KB page operation) cost nothing but their argument evaluation.
#[cfg(all(target_arch = "wasm32", debug_assertions))]
#[inline]
pub fn debug_log_1(msg: &::wasm_bindgen::JsValue) {
    web_sys::console::log_1(msg);
}

#[cfg(all(target_arch = "wasm32", not(debug_assertions)))]
#[inline]
pub fn debug_log_1(_msg: &::wasm_bindgen::JsValue) {}
