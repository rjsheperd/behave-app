...
(defn- query! []
  (let [text   (.-value (.getElementById js/document "q-input"))
        ...
        db     (d/db (:conn @state))]
    (try
      (let [results (impl-rust/q q-form db)]
...
(defn- save!
  ...
  (try
    (let [{:keys [conn db-name]} @state
          cljs-db  (d/db conn)
          rust-db  (impl-rust/sync-to-rust! cljs-db)]
      (.storeDb rust-db db-name)
      (impl-rust/set-rust-db! rust-db)
  ...
  (let [{:keys [db-name]} @state]
    (if-let [rust-db (.restoreDb WasmDataScript db-name)]
      ...
      (if-let [result (legacy-restore db-name)]
        (do
          (swap! state assoc :conn (:conn result))
          (impl-rust/set-rust-db! (:rust-db result))
