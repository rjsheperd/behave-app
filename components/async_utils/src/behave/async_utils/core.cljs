...
(defmethod call-remote! :post get [_ url data auth-token]
  (go
    (let [fetch-params {:method  "post"
                        ...
                        :body    (cond
                                   ...
                                   data                        (pr-str data)
                                   :else                       nil)}
          response     (<! (fetch (str url "?auth-token=" auth-token)
                                  fetch-params))]
      (if response
        {:success (.-ok response)
...
(defmethod call-remote! :post-text [_ url data auth-token]
  (go
    (let [fetch-params {:method  "post"
